package view.allView
{
	...
	import view.allView.shangChengView.FriendGiveComponent;
	import view.allView.shangChengView.ShuiJingPanelComponent;
	import view.allView.shangChengView.TuZhiComponent;
	import view.allView.shangChengView.ZiYuanComponent;
    ...
    public class ShangChengComponent extends Component
    {
		...
		protected function doExchangeHandler(event:MouseEvent):void
		{
			...
			for(var i:int;i<_arr.length;i++)
			{
				...
				if(tuzhiComp.exchangeBtn==event.currentTarget as Button)
				{
					dispatchEvent(new ShopEvent(ShopEvent.BUY_ITEM,true,false,i+1));
				}
			}
